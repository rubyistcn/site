...
kramdown 不是 markdown，但是，它是对 markdown 基于 Ruby 语言的重写和扩展！

kram 与 mark 的关系？有没有发现，其实就是字母顺序反转书写的关系。另外，官方站点尤其强调：kramdown 不要写成 Kramdown 或者 KramDown，正确写法只有一个——kramdown！
...
karamdown 可以创建简单的表格。使用管道符号（`|`）开始的行将被认为是表格行。如果管道符号后面紧跟着一个连接线符号（-），这就是一个分割线了。分割线用来分开一个表的表头和主体（亦可以设置表格的对齐方式），也可以用来分割表格的不同部分。如果管道符号后面跟着一个等号（=），下面的表格行就是表尾的一部分了。
